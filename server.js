...
class GameTimer {
  constructor(gameId, timeControlMinutes, playerId1, playerId2) {
    ...
    console.log(`Game timer created for ${gameId}: ${timeControlMinutes} minutes per player`);
  }
  ...
  startTimer() {
    this.timerInterval = setInterval(() => {
      ...
      if (this.currentPlayer === 1) {
        this.player1TimeRemaining = Math.max(0, this.player1TimeRemaining - elapsed);
        if (this.player1TimeRemaining <= 0) {
          this.handleTimeExpiry(this.playerId1);
          return;
        }
      } else {
        this.player2TimeRemaining = Math.max(0, this.player2TimeRemaining - elapsed);
  ...
  startRealTimeSync() {
    this.syncInterval = setInterval(() => {
      ...
      try {
        ...
        console.log(`Timer sync sent for game ${this.gameId}: P1=${this.player1TimeRemaining}s, P2=${this.player2TimeRemaining}s`);
      } catch (error) {
  ...
  switchPlayer() {
    ...
    this.lastMoveTime = Date.now();
    console.log(`Timer switched to player ${this.currentPlayer} for game ${this.gameId}`);
  ...
  handleTimeExpiry(timedOutPlayerId) {
    console.log(`Player ${timedOutPlayerId} ran out of time in game ${this.gameId}`);
    this.isActive = false;
    clearInterval(this.timerInterval);
    if (this.syncInterval) clearInterval(this.syncInterval);
    
    // Determine winner (opposite player)
    const winnerId = timedOutPlayerId === this.playerId1 ? this.playerId2 : this.playerId1;
...
async function endGameByCheckmate(gameId, winnerId, loserId, finalFen) {
  console.log(`CHECKMATE: Game ${gameId} won by player ${winnerId}, final FEN: ${finalFen}`);
  
  try {
    // Update database
    await new Promise((resolve, reject) => {
      db.run('UPDATE games SET status = ?, winner_id = ?, end_reason = ?, finished_at = CURRENT_TIMESTAMP WHERE id = ?',
        ['finished', winnerId, 'checkmate', gameId], (err) => {
          if (err) reject(err);
          else resolve();
        });
    });
    ...
    const gameOverData = {
      ...
      gameDuration: Math.floor((Date.now() - (game?.startTime || Date.now())) / 1000)
    };
    
    // Send to all players in the game room
    io.to(gameId).emit('game_over', gameOverData);
    console.log(`Checkmate notification sent to both players in game ${gameId}`);
...
async function endGameByResignation(gameId, resignedPlayerId) {
  console.log(`RESIGNATION: Player ${resignedPlayerId} resigned from game ${gameId}`);
  ...
  try {
    ...
    const winnerId = resignedPlayerId === game.playerId1 ? game.playerId2 : game.playerId1;
    
    // Update database
    await new Promise((resolve, reject) => {
      db.run('UPDATE games SET status = ?, winner_id = ?, end_reason = ?, finished_at = CURRENT_TIMESTAMP WHERE id = ?',
        ['finished', winnerId, 'resignation', gameId], (err) => {
          if (err) reject(err);
          else resolve();
        });
    });
    ...
    // Send to all players in the game room
    io.to(gameId).emit('game_over', gameOverData);
    console.log(`Resignation notification sent: ${resignedUser?.username} resigned, ${winnerUser?.username} wins`);
...
async function endGameByTimeout(gameId, winnerId, timedOutPlayerId) {
  console.log(`TIMEOUT: Player ${timedOutPlayerId} timed out in game ${gameId}, winner: ${winnerId}`);
  
  try {
    // Update database
    await new Promise((resolve, reject) => {
      db.run('UPDATE games SET status = ?, winner_id = ?, end_reason = ?, finished_at = CURRENT_TIMESTAMP WHERE id = ?',
        ['finished', winnerId, 'timeout', gameId], (err) => {
          if (err) reject(err);
          else resolve();
        });
    });
    ...
    io.to(gameId).emit('game_over', gameOverData);
    console.log(`Timeout notification sent: ${timedOutUser?.username} timed out, ${winnerUser?.username} wins`);
...
io.on('connection', (socket) => {
        ...
        [client.userId, 'inprogress', game.gameId], function(err) {
          ...
          if (creatorSocket && creatorSocket.connected) {
            ...
            const activeGame = {
              ...
              timer: new GameTimer(game.gameId, game.timeControl || 30, game.creatorId, client.userId),
              chess: new Chess(),
              startTime: Date.now()
            };
            ...
            console.log(`Match found: ${game.gameId} - ${creatorClient.username} vs ${client.username} with ${game.timeControl}min timer`);
          } else {
  ...
  socket.on('move', async (data) => {
    try {
      ...
      console.log(`Move received: ${move} from ${client.username} in game ${gameId}`);

      // Get active game and validate
      const activeGame = activeGames.get(gameId);
      if (!activeGame) {
        socket.emit('invalid_move', { reason: 'Game not found or no longer active' });
        return;
      }
      ...
      try {
        ...
        await redis.set(`game:${gameId}:turn`, activeGame.chess.turn() === 'w' ? 'white' : 'black');
      } catch (redisError) {
      ...
      const timerData = activeGame.timer ? activeGame.timer.getTimerData() : null;
      ...
      if (timerData) {
        // Include timer data relative to the player who made the move
        moveData.playerTimeRemaining = isPlayerWhite ? timerData.player1Time : timerData.player2Time;
        moveData.opponentTimeRemaining = isPlayerWhite ? timerData.player2Time : timerData.player1Time;
        moveData.currentPlayerNumber = timerData.currentPlayer;
        moveData.serverTimestamp = Date.now();
        
        console.log(`Move timer data: Player ${client.username} (${isPlayerWhite ? 'white' : 'black'}) - PlayerTime: ${moveData.playerTimeRemaining}s, OpponentTime: ${moveData.opponentTimeRemaining}s`);
      }

      // CRITICAL FIX: Check for game end IMMEDIATELY after move
      const winnerId = isPlayerWhite ? activeGame.playerId1 : activeGame.playerId2;
      const loserId = isPlayerWhite ? activeGame.playerId2 : activeGame.playerId1;
      ...
      console.log(`Move broadcasted to game room: ${moveResult.san} by ${client.username}`);
  ...
  socket.on('resign', async (data) => {
    try {
      ...
      console.log(`Player ${client.username} (ID: ${client.userId}) resigned from game ${gameId}`);
